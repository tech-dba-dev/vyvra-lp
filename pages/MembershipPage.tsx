...
const CONFETTI_COLORS = ['#CF9C97', '#EADEDA', '#A47865', '#ffffff', '#4a0422'];
...
function Confetti() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const globalFade = elapsed > 3000 ? Math.max(0, 1 - (elapsed - 3000) / 2000) : 1;
...
const COMMUNITY_URL = "https://login.circle.so/sign_up?request_host=members.vyvra.com&user%5Binvitation_token%5D=00070f91c7fda583ad16aadc8119c47e9c4d03a9-af367d04-7cbe-44ac-93d4-26f11b59abca#email";
...
export default function MembershipPage() {
  ...
  return (
    <main className="relative min-h-screen bg-brand-dark overflow-hidden selection:bg-brand-accent selection:text-brand-dark">
      <Confetti />
      ...
      <div className="relative z-10 w-full max-w-lg mx-auto px-6 py-12 md:py-20 flex flex-col items-center">
        ...
        <div
          ...
          style={animStyle(0)}
        >
          <svg className="h-10 md:h-14" viewBox="0 0 1476 430" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Divider */}
        <div
          className="w-16 h-px bg-brand-accent/30 mb-14 md:mb-20 transition-all duration-700"
          style={animStyle(400)}
        />

        {/* ── SECTION 2: Introduce the Community ── */}
        <section className="w-full text-center mb-14 md:mb-20">
          <h2
            className="font-serif text-xl md:text-3xl font-bold text-brand-beige tracking-tight mb-4 md:mb-6 transition-all duration-700"
            style={animStyle(450)}
          >
            We're building more than activewear.
          </h2>
          <p
            className="font-sans text-sm md:text-base font-light text-brand-beige/80 leading-relaxed max-w-md mx-auto transition-all duration-700"
            style={animStyle(550)}
          >
            Vyvra is growing into a community designed to support women in building sustainable habits around movement, health and accountability.
          </p>
          <p
            className="font-sans text-sm md:text-base font-light text-brand-beige/80 leading-relaxed max-w-md mx-auto mt-4 transition-all duration-700"
            style={animStyle(650)}
          >
            We're currently preparing the first phase of our structured membership experience — with guided workouts and expert-led content launching soon.
          </p>
          <p
            className="font-sans text-sm md:text-base font-semibold text-brand-accent mt-4 transition-all duration-700"
            style={animStyle(700)}
          >
            More is coming.
          </p>
        ...
        <section className="w-full text-center mb-14 md:mb-16">
          <h2
            className="font-serif text-xl md:text-3xl font-bold text-brand-beige tracking-tight mb-3 md:mb-5 transition-all duration-700"
            style={animStyle(800)}
          >
            Join the Vyvra Community
          </h2>
          <p
            className="font-sans text-sm md:text-base font-light text-brand-beige/80 leading-relaxed max-w-md mx-auto mb-8 transition-all duration-700"
            style={animStyle(850)}
          >
        ...
        {/* ── BOTTOM: Subtle Positioning Line ── */}
        <p
          className="font-sans text-xs md:text-sm font-light text-brand-accent/40 uppercase tracking-[0.3em] text-center transition-all duration-700"
          style={animStyle(1050)}
        >
